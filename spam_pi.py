...
def get_hci_devices():
    ...
    try:
        ...
        for line in result.stdout.split('\n'):
            if line.startswith('hci'): devices.append({'id': line.split(':')[0], 'manufacturer': 'Unknown'})
        ...
        with open('/proc/net/dev', 'r') as f:
            for line in f.readlines()[2:]:
                if ':' in line:
                    ...
                    if any(ifname.startswith(p) for p in ['wlan', 'wl', 'wlp']): interfaces.append({'id': ifname, 'manufacturer': 'Unknown'})
        try:
    ...
    def packet_callback(self, pkt):
        try:
            if pkt.haslayer(Dot11Beacon):
                ...
                # Method 1: Get channel from IE element
                ...
                while elt:
                    if elt.ID == 0:
                        ...
                        try: ch = int(elt.info[0])
                        except: pass
                    elt = elt.payload.getlayer(Dot11Elt)
                
                # Method 2: Fallback to RadioTap frequency
                if ch == 0 and pkt.haslayer(RadioTap):
                    try:
                        freq = pkt[RadioTap].Channel
                        if freq <= 2484: ch = (freq - 2407) // 5
                        else: ch = (freq - 5000) // 5
                    except: pass
    ...
    def scan_task(self):
        ...
        try:
            for line in proc.stdout:
                if not self.is_running: break
                line = line.strip()
                if not line or "LE Scan..." in line: continue
                parts = line.split(' ', 1)
                mac = parts[0]
                ...
                if self.parent_wifi:
                    ...
                    else: self.parent_wifi.ble_devices[mac] = BLEDevice(mac, name)
        ...
        try:
            if choice == '1' and TUI_ENABLED and wifi:
                ...
                if target_ap.channel == 0:
                    print("[!] Unknown channel. Cannot lock."); time.sleep(2); continue
                ...
                print("\n--- WPS Attacks ---"); time.sleep(2); print("[!] No vulnerable WPS APs found."); input("\nPress Enter...")
            elif choice == '7':
                print("\n--- MouseJack ---"); time.sleep(2); print("[!] No NRF24 hardware found."); input("\nPress Enter...")
